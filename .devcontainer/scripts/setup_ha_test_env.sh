...
VENV="/opt/venv"
WORKSPACE_DIR="/workspaces/ha-geist-pdu"
...
if command -v jq >/dev/null 2>&1; then
    for req in $(jq -c -r '.requirements | .[]' "${WORKSPACE_DIR}/custom_components/geist_pdu/manifest.json"); do
        ${VENV_PYTHON} -m pip install "$req"
    done
fi

echo "Ensuring pytest and HA test plugin..."
${VENV_PYTHON} -m pip install -U pytest pytest-asyncio pytest-homeassistant-custom-component ruff

echo "Setting up HA runtime directory..."
bash "${WORKSPACE_DIR}/.devcontainer/scripts/setup_homeassistant_runtime.sh"
